...
<div class="card">
  <div class="card-header">
    <router-link class="btn btn-sm btn-primary" :to="{ name: 'incidenceType'}" >
  ...
  methods:{
    ...
    sendForm: function(form) {
      ...
      if(form.target.checkValidity()){
        ...
        IncidenceService.newType(this.item)
        .then(response => {
          _this.item = response.data;
          _this.$router.push({ name: 'incidence-type-detail', params: { id: _this.item.id }});
        })
    ...
    callOnMounted: function(){
      if(this.$route.params.id){
        IncidenceService.getById(this.$route.params.id).then(response => this.item = response.data.data);
      }
    }
  },
  mounted () {
